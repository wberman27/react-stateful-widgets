...
export default function Squares() {
  ...
  const [squares, setSquares] = useState(listOfSquareIds) //squares slice of state with initial state of listofsquaresids above
  const [activeSquare, setActiveSquare] = useState(null) //activeSquares slice of state with initial state of null
  ...
  const getClassName = id => {
    ...
    return activeSquare === id ? ' active' : "" //add active class if it is the activeSquare
  };
  ...
  const markActive = id => {
    ...
    return id === activeSquare ? setActiveSquare(null) : setActiveSquare(id) //toggle activeSquare if active or not
  };
      ...
      <div className='squares'>
        {
          ...
          squares.map(id => //map over slice of state, squares
            <div
              id={id}
              key={id}
